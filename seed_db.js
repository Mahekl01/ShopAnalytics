...
const SHOP_DATA = {
  ...
  monthlyRevenue: [
    { id: "oct", month: "Oct",  revenue: 48200, orders: 38 },
    { id: "nov", month: "Nov",  revenue: 62800, orders: 51 },
    { id: "dec", month: "Dec",  revenue: 89500, orders: 74 },
    { id: "jan", month: "Jan",  revenue: 71300, orders: 58 },
    { id: "feb", month: "Feb",  revenue: 55900, orders: 44 },
    { id: "mar", month: "Mar",  revenue: 83700, orders: 67 },
    { id: "apr", month: "Apr",  revenue: 34120, orders: 20 },
  ],

  weeklyRevenue: [
    { id: "w1", week: "Week 1", revenue: 18400, orders: 14 },
    { id: "w2", week: "Week 2", revenue: 22100, orders: 17 },
    { id: "w3", week: "Week 3", revenue: 19800, orders: 16 },
    { id: "w4", week: "Week 4", revenue: 23400, orders: 20 },
  ],

  categories: [
    { id: "cat-electronics", name: "Electronics", sales: 268300, orders: 68, color: "#6366f1" },
    { id: "cat-clothing", name: "Clothing",    sales: 102900, orders: 42, color: "#ec4899" },
    { id: "cat-accessories", name: "Accessories", sales: 65400,  orders: 30, color: "#f59e0b" },
  ],
};
...
async function seed() {
  try {
    ...
    for (const c of collections) {
      console.log(`Seeding collection: ${c}...`);
      const items = SHOP_DATA[c];
      const batch = writeBatch(db);
      items.forEach(item => {
        // use item.id as doc ID if available, else let firestore auto-generate
        const ref = item.id ? doc(db, c, item.id) : doc(db, c);
        batch.set(ref, item);
      });
      await batch.commit();
